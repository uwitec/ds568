...
<div class="proctn">
    <table cellpadding=0 cellspacing=0 class="tblist">
        <tr><th colspan="2">货品</th><th>单价</th><th>数量</th><th>实付款(元)</th><th>订单状态</th><th>操作</th></tr>
        <asp:Repeater ID="Repeater1" runat="server">
            <ItemTemplate>
                <tr><td><div class="imgwrap"><img src="<%#Eval("ImgUrl") %>" onerror="javascript:this.src='http://img.china.alibaba.com/news/upload/5002027/48x48_1276134613200.gif'" onload="changeImg(this,80,80)" /></div></td><td class="proname" ><a href="http://shop<%#Eval("memberid") %>.ds568.net/template/tem1/product/product_show.aspx?pro_id=<%#Eval("productid") %>" target="_blank"><%#Eval("proname") %></a><p>时间：<%#((DateTime)Eval("CreateDate")).ToString("yyyy-MM-dd") %> 订单号：<%#Eval("OrderNum") %></p></td><td><%#Eval("price") %></td><td><%#Eval("pronum") %></td><td><%#Eval("amount")%></td><td><%#Enum.GetName(typeof(Com.DianShi.BusinessRules.Transaction.DS_Cart.State),Eval("state")) %></td><td class="act_wrap"><a  class="od_det" href="javascript:void(0);" oid="<%#Eval("OrderID") %>">详情</a><a class="od_del" dtid="<%#Eval("id") %>" href="javascript:void(0);">删除</a></td></tr>        
            </ItemTemplate>
